...
for (let p of Object.entries(game)) {
  console.log(p);
}
...
let Ind = game.team1;
let [wicketkeeper, ...fp1] = players1;
console.log(Ind, "WicketKeeper " + wicketkeeper);
console.log(fp1);
console.log(wicketkeeper, fp1);

let Aus = game.team2;
let [wcc, ...fp2] = players2;
console.log(Aus, "WicketKeeper " + wcc);
console.log(fp2);
...
for (let [keys, values] of Object.entries(game.Century)) {
  console.log(`${parseInt(keys) + 1}. ${values}`);
}

for (let i in game.Century) {
  console.log(parseInt(i) + 1);
}
for (let i = 0; i < game.Century.length; i++) {
  console.log(`${i + 1}. ${game.Century[i]}`);
}
...
let result = 0;
let sum = Object.values(game.odds);
for (let av of Object.values(game.odds)) {
  result += av;
  console.log("av => ", av);
  console.log("result => ", result / sum.length);
